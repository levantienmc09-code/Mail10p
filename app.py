...
def run_bot():
    ...
    print(f"🤖 [{time.strftime('%H:%M:%S')}] Bot đang khởi động...")
    try:
        bot.infinity_polling(timeout=60, long_polling_timeout=60)
    except Exception as e:
        print(f"❌ [{time.strftime('%H:%M:%S')}] Bot lỗi: {e}")

def restart_bot_cycle():
    """Chu kỳ restart bot mỗi 3 phút"""
    while True:
        try:
            print(f"⏳ [{time.strftime('%H:%M:%S')}] Đang chạy bot trong 3 phút...")
            time.sleep(180)  # Chạy trong 3 phút
            
            print(f"🔄 [{time.strftime('%H:%M:%S')}] Đang restart bot...")
            if bot:
                try:
                    bot.stop_polling()
                except:
            ...
            print(f"✅ [{time.strftime('%H:%M:%S')}] Bot đã được restart")
            
        except Exception as e:
            print(f"❌ [{time.strftime('%H:%M:%S')}] Lỗi restart cycle: {e}")
            time.sleep(10)

def keep_alive_server():
    """Simple HTTP server để Render không sleep"""
    ...
    print(f"🌐 [{time.strftime('%H:%M:%S')}] Keep-alive server running on port 8080")
    server.serve_forever()

def main():
    """Hàm chính"""
    print("=" * 50)
    print("🤖 BOT TELEGRAM - AUTO RESTART 3 PHÚT")
    print("=" * 50)
    print(f"⏰ Thời gian bắt đầu: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"🔄 Chu kỳ restart: 3 phút")
    print("=" * 50)
    ...
    # Chạy restart cycle trong thread riêng
    restart_thread = threading.Thread(target=restart_bot_cycle, daemon=True)
    restart_thread.start()
    
    # Chạy keep-alive server trong thread riêng
